...
function Write-Log {
    ...
    foreach ($logDir in $logRoots) {
        try {
            ...
            Add-Content -Path (Join-Path $logDir 'baidu_report.log') -Value $line -Encoding UTF8
            Write-Host $line
            return
        } catch {
            continue
    ...
    Write-Warning ('Failed to write Baidu log to project and temp directories: ' + $line)
    Write-Host $line
}
